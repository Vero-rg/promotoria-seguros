...
import { ref } from 'vue';
import { Camera, Upload, CalendarDays } from 'lucide-vue-next';
        ...
        <div class="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8 transition-all duration-300">
            <div class="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 overflow-hidden">
                ...
                <div class="px-8 py-6 border-b border-gray-100/80 bg-gray-50/30">
                    <h1 class="text-xl font-semibold text-gray-900 tracking-tight">Editar {{ type === 'promoter' ? 'Promotor' : 'Agente' }}</h1>
                    <p class="text-sm text-gray-500 mt-1">Actualiza la información del perfil del {{ type === 'promoter' ? 'promotor' : 'agente' }}.</p>
                </div>
                ...
                <form @submit.prevent="submit" class="p-8 space-y-8">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-6">
                            <div class="flex-shrink-0 relative group cursor-pointer">
                                ...
                                <div v-else class="w-20 h-20 rounded-full bg-gray-50 flex items-center justify-center border border-dashed border-gray-300 group-hover:border-gray-400 group-hover:bg-gray-100 transition-colors shadow-sm cursor-pointer" @click="fileInput?.click()">
                                    <span class="text-gray-400 font-medium text-xl tracking-tight group-hover:text-gray-500 transition-colors">{{ getInitials(form.name) }}</span>
                                </div>
                            </div>
                            <div class="flex-1">
                    ...
                    <div v-if="type === 'agent'" class="space-y-1.5">
